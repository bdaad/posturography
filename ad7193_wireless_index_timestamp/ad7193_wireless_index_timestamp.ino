...
#include <SPI.h>
#include "BluetoothSerial.h"  // ★ 追加：BluetoothSerial を使う
...
// ==== Bluetooth 設定 ====
// ★ 追加：Bluetoothで検出される名前
BluetoothSerial SerialBT;
const char* BT_NAME = "ESP32-AD7193_2";
...
void setup() {
  ...
  if (!SerialBT.begin(BT_NAME)) {
    ...
    Serial.print("[BT] Bluetooth started. Device name: ");
  ...
  Serial.println("Output format: index,timeMs,ch1,ch2,ch3,ch4 (raw code)");
...
void loop() {
  ...
  uint32_t currentIndex = ++sampleIndex;
  ...
  // 1 サンプル 1 行（index, timestamp, 4 チャンネル分）
  char line[96];
