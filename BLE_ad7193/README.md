...
| `ble_sampling_test.py` | BLE Notify の受信サンプリングレートを確認する Python スクリプト |
...
実機確認では、BLE 受信時にデータ欠落が発生することがありました。例として、2026-05-22 の確認では `total=3501` 付近で `index_drops=65` 程度の欠落が出ています。10 秒平均はおおよそ `92.5 Hz` から `98.9 Hz` でしたが、完全な連続受信ではありません。

## サンプリングレート確認

CSV 保存ではなく、BLE の受信レートだけを見たい場合は以下を使います。
...
```text
[15:30:12] 1秒間のサンプル数: 96 samples (約 95.8 Hz)
[15:30:21] 10秒間のサンプル数: 961 samples (平均 96.1 Hz, 累計 961 samples, index欠落 0)
```

`index欠落` が 0 でない場合は、BLE 通知の一部が PC 側で受信できていないことを意味します。BLE の受信状態は PC、OS、Bluetooth アダプタ、周囲の電波状況で変わるため、測定前に毎回このスクリプトで確認してください。

主なオプション:

```bash
python3 ble_sampling_test.py --duration 30
python3 ble_sampling_test.py --print-samples
python3 ble_sampling_test.py --address <BLE address or UUID>
```
...
- BLE ではデータが落ちることがあります。`index_drops` または `index欠落` が増える場合、その分のサンプルは欠落しています。
- 校正など取りこぼしを避けたい本番測定では、BLE ではなく USB シリアル版の `ad7193_wired/ad7193_wired.ino` を使ってください。
